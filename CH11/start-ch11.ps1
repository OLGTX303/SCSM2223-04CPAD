...
function Initialize-Database {
    ...
    Write-Host 'Loading Chapter 11 MySQL + users schema...'
...
if ($envText -match 'JWT_SECRET=change-me') {
    $secret = & $PhpPath -r "echo bin2hex(random_bytes(32));"
    $envText = $envText -replace 'JWT_SECRET=.*', "JWT_SECRET=$secret"
...
if (!$SkipInstall) {
    if (!(Test-Path (Join-Path $Root 'vendor'))) {
        ...
        Write-Host 'Installing Chapter 11 PHP dependencies...'
        & $PhpPath $ComposerPhar install --working-dir="$Root"
    }

    if (!(Test-Path (Join-Path $Frontend 'node_modules'))) {
        Write-Host 'Installing Chapter 11 frontend dependencies...'
...
if (Test-PortInUse $ApiPort) {
    Write-Warning "Port $ApiPort is already in use. Stop the other API server before starting Chapter 11."
} else {
    Start-Terminal `
        -Title 'Chapter 11 API' `
...
Write-Host "Chapter 11 API:      http://localhost:$ApiPort/"
Write-Host "Chapter 11 frontend: $url"
